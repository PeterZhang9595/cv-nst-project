...
# 分支的新建与合并
*注意：切换分支的时候，Git会重置工作目录，使其看起来像回到了在对应分支上最后一次提交的样子。*

## 关于合并
比方说你在temp_branch分支上完成了一些修改，现在想把这些修改合并到main分支上，可以```git switch main```，然后```git merge temp_branch```。

- fast forward
如果两个待合并的分支是直接连通的，那么这时候进行合并就不会存在需要解决的分歧。

- diverged
如果两个待合并的分支不是直接连通，而是位于它们的公共祖先的两条分叉上，那么此时Git会使用待合并的C4,C5，与他们的共同祖先C2进行一个简单的三方合并。

这时就会把三方合并的结果创建一个新的快照提交指向它。

剩余部分都是一些通俗易懂的例子，可以进一步阅读Pro Git，这里就不赘述了。
